...
<head>
	...
	<script type="text/javascript" src="js/buy.js" defer></script>
		...
		<div class="content">
			<?php 
				...
				while($items = mysqli_fetch_assoc($query)){ ?>
					<div class="cart">
						<a href="element.php?id=<?php echo $items['id']?>"><div class="cart-img"><img src=<?php echo $items['image'] ?> alt=""></div><h2><?php echo $items['title'] ?></h2><h4><?php echo $items['short'] ?></h4></a>
						<button onclick="swal('Круто!', 'Добавлено в корзину', 'success');" id=<?php echo '"b'.$items['id'].'"'?> class = "button-buy subs subscription button-buy">Buy</button>
					</div>
